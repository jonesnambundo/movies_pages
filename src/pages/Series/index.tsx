...
// Config TMDB (mesma chave/base que você já usa)
const API_KEY = "f975b4f5e10040b0ed800db3826ac8bc";
const TMDB_BASE = "https://api.themoviedb.org/3";
const IMG_BASE = "https://image.tmdb.org/t/p/w500";

const TV_URLS = {
  TRENDING: `${TMDB_BASE}/trending/tv/week?api_key=${API_KEY}&language=pt-BR`,
  POPULAR: `${TMDB_BASE}/tv/popular?api_key=${API_KEY}&language=pt-BR`,
  TOP_RATED: `${TMDB_BASE}/tv/top_rated?api_key=${API_KEY}&language=pt-BR`,
};

// Tipagem para TV
interface TvShow {
  id: number;
  name: string;               // título para séries
  poster_path: string | null;
  vote_average: number;
  first_air_date?: string;    // data de estreia
}
  ...
  return (
    <div className="min-w-[160px] sm:min-w-[180px] md:min-w-[200px] lg:min-w-[220px] xl:min-w-[240px] snap-start">
      ...
      <div className="px-1 pt-3">
        <h3 className="text-amber-500 text-sm font-medium truncate">{show.name}</h3>
...
type TvRowProps = {
  ...
  subtitle?: string;
  fetchUrl: string;
  id?: string;       // id do <section> para âncoras (#popular, etc.)
...
function TvRow({ title, subtitle, fetchUrl, id, limit = 18 }: TvRowProps) {
  ...
  const handleDetails = (id: number) => {
    console.log("ver detalhes (TV) -> id:", id);
    // Dica: quando criar a página de detalhes de série:
    // navigate(`/detalhes-serie/${id}`);
  };
